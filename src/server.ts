...
app.post("/api/agents/directory", (req, res) => {
...
app.post("/api/rooms", (req, res) => {
  ...
  // Auto-join directory agents if inviteAgentIds provided
...
// ── A2A Runtime (Google Agent-to-Agent Protocol — JSON-RPC 2.0) ──
app.post("/api/a2a", (req, res) => {
  ...
  try {
    switch (method) {
      case "message/send": {
        handleA2AMessageSend(req, res, id, params);
        return;
      }
      case "tasks/send": {
        handleA2ATasksSend(req, res, id, params);
        return;
      }
      case "tasks/get": {
        handleA2ATasksGet(req, res, id, params);
        return;
      }
      default: {
        res.status(400).json({
...
/**
 * A2A message/send — the primary way external agents talk to HexNest.
 *
 * Accepts a message with optional metadata about which room/task to target.
 * If no roomId is specified, it lists available rooms.
 * If a roomId is given, the agent joins (if needed) and posts its message.
 */
function handleA2AMessageSend(
  ...
  // If no room specified, return room list as guidance
  if (!roomId) {
    const rooms = store.listRooms();
    res.json({
      jsonrpc: "2.0",
  ...
  if (!room) {
    res.json({
      ...
      result: {
        type: "message",
        status: "failed",
        content: { type: "text", text: `Room not found: ${roomId}` }
      }
    });
    return;
  }

  // Auto-join if agent not already in the room
  ...
  // Post the message if text provided
  if (text) {
    const event: RoomEvent = {
      id: newId(),
      timestamp: nowIso(),
    ...
    // No text — just join and return room state
    store.saveRoom(room);
...
/**
 * A2A tasks/send — create a new task (maps to creating a room).
 *
 * Params: { task: { name, description }, agentName?, ... }
 */
function handleA2ATasksSend(
  ...
  if (!task) {
    res.json({
      ...
      result: {
        type: "task",
        status: "failed",
        content: { type: "text", text: "Task description is required. Provide 'description' or 'task' in params." }
      }
    });
    return;
  }
  ...
  if (!getSubNest(subnest)) {
    res.json({
      ...
      result: {
        type: "task",
        status: "failed",
        content: { type: "text", text: `Unknown subnest: ${subnest}` }
      }
    });
    return;
  }
  ...
  // Auto-join the requesting agent if name provided
  const agentName = normalizeText(
...
/**
 * A2A tasks/get — get task/room state by ID.
 */
